...
namespace OpdexV1Core.Tests.Pools
{
    public class OpdexMiningPoolTests : TestBase
    {
            ...
            UInt256 rewardRate, UInt256 expected)
        {
            ...
            State.SetUInt256(nameof(IOpdexMiningPool.RewardPerStakedTokenLast), currentRewardPerToken);
            ...
            UInt256 rewardRate, UInt256 expected)
        {
            ...
            State.SetUInt256(nameof(IOpdexMiningPool.RewardPerStakedTokenLast), rewardPerToken);
            State.SetUInt256(nameof(IOpdexMiningPool.TotalSupply), totalSupply);
            State.SetUInt256(nameof(IOpdexMiningPool.RewardRate), rewardRate);
            State.SetUInt64(nameof(IOpdexMiningPool.LastUpdateBlock), lastUpdateBlock);
            State.SetUInt64(nameof(IOpdexMiningPool.MiningPeriodEndBlock), periodFinish);
            ...
            UInt256 rewardRate, UInt256 amount, UInt256 userRewardPerTokenPaid)
        {
            ...
            VerifyLog(new MineLog { Miner = Miner1, Amount = amount, TotalSupply = expectedTotalSupply, EventType = MineEventType.StartMining }, Times.Once);
        }
            ...
            UInt256 minerWithdraw, UInt256 expectedReward)
        {
            ...
            VerifyLog(new MineLog { Miner = Miner1, Amount = minerWithdraw, TotalSupply = expectedTotalSupply, EventType = MineEventType.StopMining}, Times.Once);
        ...
        private void StartMining(IOpdexMiningPool miningPool, Address miner, UInt256 amount)
        {
            ...
            VerifyLog(new MineLog {Miner = miner, Amount = amount, TotalSupply = expectedTotalSupply, EventType = MineEventType.StartMining}, Times.Once);
        }
        ...
        private void StopMining(IOpdexMiningPool miningPool, Address miner, UInt256 amount, UInt256 rewards)
        {
            ...
            VerifyCall(StakingToken, 0, nameof(IOpdexPool.TransferTo), new object[] {miner, rewards}, Times.AtLeastOnce);
            VerifyLog(new CollectMiningRewardsLog { Miner = miner, Amount = rewards }, Times.AtLeastOnce);
            VerifyLog(new MineLog { Miner = miner, Amount = amount, TotalSupply = expectedTotalSupply, EventType = MineEventType.StopMining }, Times.AtLeastOnce);
        }
